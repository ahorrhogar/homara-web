...
export async function generateMetadata({
  ...
  const t = await getTranslations("guides.mejores-sombrillas-resistentes-amazon-2026");
  const title = t("title");
...
const umbrellaData: UmbrellaData[] = [
  {
    rank: 1,
...
export default async function GuidePage({
  ...
  const productCopy = t.raw("products") as Array<{
    bestFor: string;
    notes: string[];
    pros: string[];
    cons: string[];
  ...
  const umbrellaProducts = umbrellaData.map((data, index) => ({
    ...data,
    ...productCopy[index],
  }));
        ...
        <article className="pb-16">
          <header className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-accent/15 via-secondary/50 to-card p-6 md:p-10">
            ...
            <div className="relative max-w-4xl">
              ...
              <div className="mt-6 flex flex-wrap gap-2 text-xs">
                ...
                <span className="rounded-full bg-card px-3 py-1 font-medium text-foreground">{t("metaPrice")}</span>
              </div>
            </div>
          </header>
              ...
              <table className="w-full min-w-[1040px] text-sm">
                <thead>
                  <tr className="border-b border-border bg-secondary/50 text-left">
                    ...
                    <th className="px-4 py-3 font-semibold text-foreground">{t("thTamano")}</th>
                    <th className="px-4 py-3 font-semibold text-foreground">{t("thMaterial")}</th>
                    <th className="px-4 py-3 font-semibold text-foreground">{t("thUV")}</th>
                    <th className="px-4 py-3 font-semibold text-foreground">{t("thPeso")}</th>
                  ...
                  {umbrellaProducts.map((product) => (
                    <tr key={product.rank} className="border-b border-border last:border-b-0">
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          {t("ctaVerOferta")} <ExternalLink className="h-3.5 w-3.5" />
                        </a>
                      </td>
                    </tr>
                  ))}
                    ...
                    <div>
                      <div className="grid gap-3 text-sm md:grid-cols-2 lg:grid-cols-4">
                        <div className="rounded-lg bg-secondary/50 p-3">
                          <p className="text-xs text-muted-foreground">{t("specTamano")}</p>
                          <p className="font-semibold text-foreground">{product.size}</p>
                        </div>
                        <div className="rounded-lg bg-secondary/50 p-3">
                          <p className="text-xs text-muted-foreground">{t("specMaterial")}</p>
                          <p className="font-semibold text-foreground text-xs">{product.material}</p>
                        </div>
                        <div className="rounded-lg bg-secondary/50 p-3">
                          <p className="text-xs text-muted-foreground">{t("specUV")}</p>
                          <p className="font-semibold text-foreground">{product.uvProtection}</p>
                        </div>
                        <div className="rounded-lg bg-secondary/50 p-3">
                          <p className="text-xs text-muted-foreground">{t("specPeso")}</p>
                      ...
                      <div className="mt-4 grid gap-4 md:grid-cols-3">
                        <div className="rounded-lg border border-border bg-background p-3">
                          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("whyLabel")}</p>
          ...
          <section className="mt-12 rounded-2xl border border-border bg-card p-5 md:p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <Link href="/buscar?q=sombrilla" className="rounded-lg border border-border bg-background p-3 text-sm font-medium text-foreground hover:bg-secondary">{t("linkBuscarSombrilla")}</Link>
              <Link href="/buscar?q=parasol%20terraza" className="rounded-lg border border-border bg-background p-3 text-sm font-medium text-foreground hover:bg-secondary">{t("linkBuscarParasol")}</Link>
              <Link href="/blog/10-mesas-de-terraza-baratas-y-bonitas-en-amazon-2026" className="rounded-lg border border-border bg-background p-3 text-sm font-medium text-foreground hover:bg-secondary">{t("linkMesasBlog")}</Link>
